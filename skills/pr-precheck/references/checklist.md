...
**在 `gh pr create` / `gh pr close` / `gh pr comment` 等会被外部看到的命令之前，必须：**

- [ ] 把完整草稿（标题 + body + 目标 PR 号 + 操作类型）呈现给用户
- [ ] 明确说出"准备执行 X，确认后动手"
- [ ] 等到用户给出明确"动手 / 提交 / 确认"等指令
- [ ] **不靠 autonomous mode 默认通过**——autonomous 只授权探索性操作，不授权对外广播

...
> **注意**：以下示例使用默认的 streamich/git-cz 风格（emoji 在 type 和 description 之间）。
...
如果项目检测到 cz-git 配置，emoji 会出现在 type 之前：
